...
import { createRideEmailPdf } from './generatePDF.utils';
import { ICashoutRequests } from 'src/models/cashoutRequest.model';

interface EmailConfig extends TransportOptions {
  host: string;
  port: number;
  secure: boolean;
  auth: { user: string; pass: string };
  tls?: { rejectUnauthorized: boolean; ciphers?: any };
  debug?: boolean;
}
const logoUrl = 'https://piupiu-storage.s3.us-west-2.amazonaws.com/image/images/logo-text.svg';

export async function sendDocumentVerificationMail(
  user: IUser,
  verificationStatus: VerificationResult,
) {
  const transporterOptions: EmailConfig = {
    host: emailConfig.emailHost,
    port: Number(emailConfig.emailPort),
    secure: emailConfig.emailSecure === 'true', // false for STARTTLS
    auth: {
  ...
  const transporter: Transporter<SendMailOptions> = nodemailer.createTransport(transporterOptions);
  let rejectedKey;
  let reason;
...
export async function sendContactUsMail(email: string, content: string) {
  const transporterOptions: EmailConfig = {
    host: emailConfig.emailHost,
    port: Number(emailConfig.emailPort),
    secure: emailConfig.emailSecure === 'true', // false for STARTTLS
...
export async function sendRideCompletionMail(user: IUser, content: IRide) {
  const transporterOptions: EmailConfig = {
    host: emailConfig.emailHost,
    port: Number(emailConfig.emailPort),
    secure: emailConfig.emailSecure === 'true', // false for STARTTLS
  ...
  const pdfBuffer = await createRideEmailPdf(user, content);
...
export async function sendCashOutRequest(email: string, content: ICashoutRequests) {
  const transporterOptions: EmailConfig = {
    host: emailConfig.emailHost,
    port: Number(emailConfig.emailPort),
    secure: emailConfig.emailSecure === 'true', // false for STARTTLS
...
export async function sendCareerApplicationMail(args: Record<string, unknown>) {
  const transporterOptions: EmailConfig = {
    host: emailConfig.emailHost,
    port: Number(emailConfig.emailPort),
    secure: emailConfig.emailSecure === 'true',
    auth: {
      user: emailConfig.emailUser,
      pass: emailConfig.emailpassword,
    },
  ...
  try {
    await transporter.sendMail(mailOptions);
    logger.info('Career application reply mail sent successfully');
  } catch (error) {
    logger.error('Error sending career application reply mail:', error);
  }
}
