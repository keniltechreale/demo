...
router.post(
  '/available/categories/:ride_id',
  AuthGuard.verifyUserAccessToken,
  CustomerController.AvailableVehicleCategories,
);

router.post(
  '/instructions/:ride_id',
  AuthGuard.verifyUserAccessToken,
  upload.single('passengersAudioInstructions'),
  singleFileUpload,
  CustomerController.addInstructions,
);

router.post(
  '/select/drivers/:ride_id',
  AuthGuard.verifyUserAccessToken,
  CustomerController.selectDrivers,
);

router.post(
  '/coupons/redeem/:ride_id',
  AuthGuard.verifyUserAccessToken,
  ValidationMiddleware.validate(ValidationMiddleware.schema.RedeemCoupon),
  CustomerController.redeemCoupon,
);

router.post(
  '/coupons/:type/:ride_id',
  AuthGuard.verifyUserAccessToken,
  ValidationMiddleware.validate(ValidationMiddleware.schema.RedeemCoupon),
  CustomerController.VerifyCoupon,
);
