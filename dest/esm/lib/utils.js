...
import CountryModel from '../models/countrydata.model';
 ...
 */
export function sendErrorResponse(data) {
 ...
 */
export function sendSuccessResponse(_a) {
...
export function checkVerification(oldObject, documents) {
    const oldObjectMap = {};
    const rejectedKeys = {};
...
export function docVerification(arr) {
    let hasRejected = false;
    const rejectedKeys = {};
...
export const updateCountrySymbols = () => __awaiter(void 0, void 0, void 0, function* () {
    const countries = yield CountryModel.findAll({ raw: true });
    for (const country of countries) {
        ...
        console.log('----------------->', symbol);
        if (symbol) {
            yield CountryModel.update({ symbol }, { where: { countryCode: country.countryCode } });
        }
    }
    return;
});
